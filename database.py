...
class crub:
    def __init__(self, sql, database=None,  **kwargs):
        ...
        self.conn.execute("CREATE TABLE IF NOT EXISTS filters(id INTEGER, word TEXT, caption TEXT, file_id TEXT, file_type TEXT);")
        self.conn.save()
    ...
    def get_flood(self, cid):
        r = self.conn.execute(f"SELECT (amount) FROM flood WHERE id = '{cid}';")
        return r

    # Filter
    def add_filter(self, cid, key, caption="", file_id="", file_type=""):
        r = self.conn.execute(f"SELECT * FROM filters WHERE id = '{cid}' AND word = '{key}';")
        if r:
            self.conn.execute(f"UPDATE filters SET caption = '{caption}', file_id = '{file_id}', file_type = '{file_type}' WHERE id = '{cid}' AND word = '{key}';")
        else:
            self.conn.execute(f"INSERT INTO filters VALUES ('{cid}', '{key}', '{caption}', '{file_id}', '{file_type}');")
        self.conn.save()
    ...
    def get_filter(self, cid, key):
        r = self.conn.execute(f"SELECT caption, file_id, file_type FROM filters WHERE id = '{cid}' AND word = '{key}';")
        return r
